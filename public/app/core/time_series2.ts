...
export default class TimeSeries {
  ...
  formatValueByName(valueName, value) {
    if (valueName === 'pct') {
      return value + '%';
    }
  ...
  static addPctStats(series, valueName, total, decimal) {
    const pct = (series.stats[valueName] / total * 100).toFixed(decimal);
    series.stats.pct = pct ? parseFloat(pct) : 0;
    return series;
  }

  static updateLegendPercentage(data: TimeSeries[], panel) {
    if (!panel) {
      return;
    }

    let total = undefined;
    const { valueName } = panel;
    total = data.reduce((acc, obj) => acc + obj.stats[valueName], 0);

    let decimal = 0;
    if (panel.legend.percentageDecimals) {
      decimal = panel.legend.percentageDecimals;
    }

    if (total) {
      data = data.map(series => TimeSeries.addPctStats(series, valueName, total, decimal));
    }
  }
}
