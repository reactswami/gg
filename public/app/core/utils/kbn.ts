...
import _drilldown from './drilldown';
...
kbn.valueFormats.con-gm3 = kbn.formatBuilders.fixedUnit('-g/m-');
kbn.valueFormats.con-gNm3 = kbn.formatBuilders.fixedUnit('-g/Nm-');
...
kbn.valueFormats.megahertz = kbn.formatBuilders.decimalSIPrefix('Hz', 2);
...
kbn.toDuration = (size, decimals, timeScale, mode = 'long') => {
  if (size === null) {
    return '';
  }
  if (size === 0) {
  ...
  if (size < 0) {
    return kbn.toDuration(-size, decimals, timeScale, mode) + ' ago';
  }
  ...
  for (let i = 0; i < units.length && decimals >= 0; i++) {
    ...
    if (value >= 1 || decrementDecimals) {
      ...
      const unit = mode === 'long' ? ' ' + units[i].long + (floor !== 1 ? 's' : '') : units[i].short;
...
kbn.valueFormats.dtdurationshort = (size, decimals) => {
  return kbn.toDuration(size, decimals, 'second', 'short');
};
...
kbn.valueFormats.dateTimeAsIso = epoch => {
  const time = moment.tz(moment.unix(epoch), dashboard_data.timezone);

  if (moment.tz(moment(), dashboard_data.timezone).isSame(moment.unix(epoch), 'day')) {
    return time.format('HH:mm:ss');
  }
  return time.format('YYYY-MM-DD HH:mm:ss');
};

kbn.valueFormats.dateTimeAsUS = epoch => {
  const time = moment.tz(moment.unix(epoch), dashboard_data.timezone);
...
kbn.valueFormats.dateTimeAsAU = epoch => {
  const time = moment.tz(moment.unix(epoch), dashboard_data.timezone);
...
kbn.valueFormats.dateTimeFromNow = (epoch, isUtc) => {
  const time = isUtc ? moment.unix(epoch).tz('UTC') : moment.tz(moment.unix(epoch), dashboard_data.timezone);
  return time.fromNow();
};

kbn.valueFormats.dateTimeFromNowMs = (epoch, decimals, isUtc) => {
  const time = isUtc ? moment.unix(epoch).tz('UTC') : moment.tz(moment.unix(epoch), dashboard_data.timezone);
  const duration = moment(moment.now()).diff(time, 'milliseconds');
  return kbn.toDuration(duration, decimals || 0, 'millisecond');
};

kbn.valueFormats.dateTimeFromNowShort = (epoch, decimals, isUtc) => {
  const time = isUtc ? moment.unix(epoch).tz('UTC') : moment.tz(moment.unix(epoch), dashboard_data.timezone);
  const duration = moment(moment.now()).diff(time, 'seconds');
  return kbn.toDuration(duration, decimals || 0, 'second', 'short');
};

kbn.valueFormats.dateTimeFromNowHMS = (epoch, isUtc) => {
  const time = isUtc ? moment.unix(epoch).tz('UTC') : moment.tz(moment.unix(epoch), dashboard_data.timezone);
  const duration = moment(moment.now()).diff(time, 'seconds');
  return kbn.secondsToHhmmss(duration);
};
  ...
  return [
    ...
    /*{
      ...
      submenu: [
        ...
        { text: 'Icelandic Kr-na (kr)', value: 'currencyISK' },
        ...
        { text: 'Polish Z-oty (PLN)', value: 'currencyPLN' },
    ...
    },*/
    {
      ...
      submenu: [
        ...
        { text: 'MegaHertz (MHz)', value: 'megahertz' },
        ...
        { text: 'duration short (s)', value: 'dtdurationshort' },
    ...
    },
    {
      ...
      submenu: [
        ...
        { text: 'MM/DD/YYYY h:mm:ss a', value: 'dateTimeAsUS' },
        { text: 'DD/MM/YYYY h:mm:ss a', value: 'dateTimeAsAU' },
        { text: 'From Now', value: 'dateTimeFromNowMs' },
        { text: 'From Now (short)', value: 'dateTimeFromNowShort' },
        { text: 'From Now (hh:mm:ss)', value: 'dateTimeFromNowHMS' },
        { text: 'From Now (relative)', value: 'dateTimeFromNow' },
      ],
    },
    {
      text: 'data (IEC)',
    ...
    },
    {
      ...
      submenu: [
        ...
        { text: 'microgram per cubic metre (-g/m-)', value: 'con-gm3' },
        { text: 'microgram per normal cubic metre (-g/Nm-)', value: 'con-gNm3' },
...
kbn.getDecimalsForValue = (decimals: number, value: number): { decimals: number; scaledDecimals: number } => {
  if (_.isNumber(decimals)) {
    return { decimals: decimals, scaledDecimals: null };
  }
