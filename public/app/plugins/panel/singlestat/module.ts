...
class SingleStatCtrl extends MetricsPanelCtrl {
  ...
  coloringTypes = [{ text: 'Range', value: 'range' }, { text: 'Regex', value: 'regex' }];
      ...
      for (let i = 0; i < this.panel.valueMaps.length; i++) {
        ...
        if (map.value === 'null') {
          if (data.value === null || data.value === void 0) {
            data.valueFormatted = this.templateSrv.replace(String(map.text));
            return;
          }
          continue;
        }
        ...
        if (value === data.valueRounded) {
          data.valueFormatted = this.templateSrv.replace(String(map.text));
          return;
        } else if (map.value === data.valueFormatted) {
          data.valueFormatted = this.templateSrv.replace(String(map.text));
          return;
        }
      }
  ...
  addColoring() {
    if (!this.panel.coloring) {
      this.panel.coloring = [];
    }
    this.panel.coloring.push({
      type: 'range',
      from: '',
      to: '',
      regex: '',
  ...
  removeColoring(index) {
    this.panel.coloring.splice(index, 1);
    this.render();
  }
  ...
  link(scope, elem, attrs, ctrl) {
    ...
    function applyColoringThresholds(valueString) {
      const color = getColorForValue(data, data.value, valueString, templateSrv);
      if (color) {
    ...
    function getValidRanges(list, start, end) {
      let ranges = [];
      start = Number(templateSrv.replace(String(start)));
      end = Number(templateSrv.replace(String(end)));
      if (isNaN(start) || isNaN(end)) {
        return [];
      }

      for (let i = 0; i < list.length; i++) {
        if (start >= list[i].end || end <= list[i].start) {
          /* No overlap with this range */
          continue;
        }

        if (start < list[i].start) {
          /* Current range starts before exiting one */
          if (end > list[i].end) {
            /* Existing range entirely within current range, so split */
            ranges = ranges.concat(getValidRanges(list, list[i].end, end));
          }

          /* Clear anything taken by existing range */
          end = list[i].start;
        } else {
          /* Clear anything taken by existing range */
          if (end <= list[i].end) {
    ...
    function addGauge() {
      ...
      for (let i = data.coloring.length - 1; i >= 0; i--) {
        if (data.coloring[i].type !== 'range' || !data.coloring[i].from || !data.coloring[i].to) {
          continue;
        } else if (Number(templateSrv.replace(String(data.coloring[i].from))) < Number(data.coloring[i].to)) {
          colorRanges = colorRanges.concat(
            getValidRanges(colorRanges, data.coloring[i].from, data.coloring[i].to).map(r => ({
              ...r,
              color: data.coloring[i].color,
            }))
          );
        }
      }

      let colors = colorRanges.sort((a, b) => a.start - b.start);
      for (let i = colors.length - 2; i >= 0; i--) {
        if (colors[i].end < colors[i + 1].start) {
          colors.splice(i + 1, 0, {
            start: colors[i].end,
            end: colors[i + 1].start,
            color: '#D3D3D3',
          });
        }
      }
      ...
      if (colors.length === 1) {
        minValue = colors[0].start;
        maxValue = colors[0].end;
        colors = [
          {
            value: colors[0].start,
        ...
        minValue = colors.length > 0 ? colors[0].start : null;
        maxValue = colors.length > 0 ? colors[colors.length - 1].end : null;
        colors = colors.map(r => ({
          value: r.end,
          color: r.color,
        }));
      ...
      // Reduce gauge width if labels enabled
      const gaugeWidthReduceRatio = panel.gauge.labels ? 1.5 : 1;
        ...
        series: {
          gauges: {
            ...
            threshold: {
              values: colors,
              label: {
                show: panel.gauge.labels,
                margin: thresholdMarkersWidth + 1,
                font: { size: thresholdLabelFontSize },
              },
              show: panel.gauge.markers,
              width: thresholdMarkersWidth,
            },
            value: {
              color: panel.colorValue
                ? getColorForValue(data, data.valueRounded, data.valueFormatted, templateSrv)
                : null,
              formatter: () => {
                return getValueText();
              },
    ...
    function render() {
      ...
      data.coloring = panel.coloring;

      const body = panel.gauge.show ? '' : getBigValueHtml();

      if (panel.colorBackground) {
        const color = getColorForValue(data, data.value, data.valueFormatted, templateSrv);
        if (color) {
    ...
    function hookupDrilldownLinkTooltip() {
      ...
      elem.mousemove(e => {
        ...
        drilldownTooltip.text(linkInfo.title);
...
function getColorForValue(data, value, valueString = null, templateSrv?) {
  if (!data.coloring) {
    return null;
  }

  for (let i = data.coloring.length - 1; i >= 0; i--) {
    if (data.coloring[i].type === 'range') {
      if (
        (data.coloring[i].from === '' || Number(templateSrv.replace(String(data.coloring[i].from))) <= Number(value)) &&
        (data.coloring[i].to === '' || Number(templateSrv.replace(String(data.coloring[i].to))) >= Number(value))
      ) {
        return data.coloring[i].color;
      }
    } else if (data.coloring[i].type === 'regex') {
      value = valueString !== null ? valueString : Number(value);
      const regex = kbn.stringToJsRegex(templateSrv.replace(data.coloring[i].regex));
