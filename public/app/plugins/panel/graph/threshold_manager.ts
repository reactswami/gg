...
export class ThresholdManager {
  ...
  renderHandle(handleIndex, defaultHandleTopPos) {
    const model = this.thresholds[handleIndex];
    const value = Number(this.panelCtrl.templateSrv.replace(String(model.value)));
  ...
  addFlotOptions(options, panel) {
    ...
    for (i = 0; i < panel.thresholds.length; i++) {
      ...
      const value = Number(this.panelCtrl.templateSrv.replace(String(threshold.value)));
      if (!_.isNumber(value)) {
        continue;
      }
      ...
      if (threshold.fill) {
        if (threshold.yaxis === 'right' && this.hasSecondYAxis) {
          options.grid.markings.push({
            y2axis: { from: value, to: limit },
            color: fillColor,
          });
        } else {
          options.grid.markings.push({
            yaxis: { from: value, to: limit },
            color: fillColor,
          });
        }
      }
