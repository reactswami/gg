import $ from 'jquery';
import { appEvents, TimeSeries } from 'app/core/core';
...
export default function GraphTooltip(this: any, elem, dashboard, scope, getSeriesFn) {
  ...
  function formatValue(series: TimeSeries, value: number): string {
    const format = kbn.valueFormats[panel.yaxes[series.yaxis - 1].format];
    const decimals = panel.decimals;
    const decimalInfo = kbn.getDecimalsForValue(decimals, value);
    return format ? format(value, decimalInfo.decimals, decimalInfo.scaledDecimals) : series.formatValue(value);
  }
  ...
  this.show = (pos, item) => {
    ...
    if (panel.tooltip.timeFormat && panel.tooltip.timeFormat !== 'custom') {
      tooltipFormat = panel.tooltip.timeFormat;
      if (seriesList[0].hasMsResolution) {
        tooltipFormat += ' %H:%M:%S.SSS';
      } else {
        tooltipFormat += ' %H:%M:%S';
      }
    } else {
      tooltipFormat = panel.tooltip.customTimeFormat || '%d/%b/%y %I:%M%p';
    }
    ...
    if (allSeriesMode) {
      ...
      absoluteTime = ctrl.formatDate(seriesHoverInfo.time, tooltipFormat);
      ...
      for (i = 0; i < seriesHoverInfo.length; i++) {
        ...
        seriesHtml += `
            <div class='graph-tooltip-list-item ${highlightClass}'>
               <div class='graph-tooltip-series-name'>
                  <i class='fa fa-minus' style='color:${hoverInfo.color};'></i>
                  ${hoverInfo.label}:
               </div>
      ...
      value = formatValue(series, value);

      absoluteTime = ctrl.formatDate(item.datapoint[0], tooltipFormat);
