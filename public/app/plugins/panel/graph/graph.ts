...
import Legend, {
  ...
  LegendDrillDownState,
} from 'app/core/components/Legend/Legend';
...
class GraphElement {
  ...
  static $injector2: any;

  /** @ngInject */
  constructor(private scope, private elem, private timeSrv, $injector) {
    this.ctrl = scope.ctrl;
    this.dashboard = this.ctrl.dashboard;
    this.panel = this.ctrl.panel;
    GraphElement.$injector2 = $injector;
    ...
    this.elem.bind('plotcontinue', (event, pos, item) => {
      if (this.panel.timeFrom) {
        this.plot.cancelSelection = true;
      }
    });
  ...
  static getDrilldownLink(label: string, drilldown: DrillDownOptions) {
    const linkSrv = GraphElement.$injector2.get('linkSrv');
    return _drillDown.getDrilldownLink(label, drilldown, linkSrv);
  }

  static getVariableValue(drilldown: any, label: string) {
    const templateSrv = GraphElement.$injector2.get('templateSrv');
    return templateSrv.replace(_drillDown.getLabelFromDrilldown(drilldown, label));
  }


  /*
   * Renders the graph data for
   * 1. Dashboard and panel refresh
   * 2. Toggling in progress
   * 3. Toggling while query modification in progress.
   * isToggling - Track if the legend is in the toggle mode or not,
  */
  renderGraphData(isToggling: boolean) {
    const { hiddenSeries } = this.ctrl;
    const hiddenSeriesId = Object.keys(hiddenSeries).filter(key => !hiddenSeries[key]);
    if (hiddenSeriesId?.length > 0) {
      // While toggling, render needs to handle query modification as well.
      if (isToggling) {
        // if render is from the toggling then use the cached data to filter
        this.data = this.legendData.filter(data => hiddenSeriesId.includes(data.id));
        this.buildFlotPairs(this.data);
      } else {
        // Handling query modification while toggle is in progress
        // which requires saving of cached data.
        this.legendData = this.data;
        this.buildFlotPairs(this.data);
        this.data = this.data.filter(data => hiddenSeriesId.includes(data.id));
        this.buildFlotPairs(this.data);
      }
    } else {
      // Rendering when the panel or dashboard is refreshed
      // Restore the cached data only while toggling is in progress
      if (this.legendData && isToggling) {
        this.data = this.legendData;
      }
      this.buildFlotPairs(this.data);
      this.legendData = this.data;
    }
  ...
  onRender(renderData) {
    ...
    this.renderGraphData(!renderData);
    ...
    const {
      showLegend: showDrillDown,
      url: drillDownUrl,
      targetBlank: newWindow,
      type: drillDownType,
    } = this.panel.drilldown;
    const legendOptions = { asTable, rightSide, sideWidth, sort, sortDesc, hideEmpty, hideZero, show, minCharacters };
    const valueOptions = { values, min, max, avg, current, total };
    const drillDownLegendState: LegendDrillDownState = {
      drillDownUrl,
      showDrillDown,
      newWindow,
      drillDownType,
      getLinkContent: GraphElement.getDrilldownLink,
      drilldown: this.panel.drilldown,
      getVariableValue: GraphElement.getVariableValue,
    };

    const { minHeader, maxHeader, currentHeader, totalHeader, avgHeader } = this.panel.legend;
    const tableDisplayProps = { minHeader, maxHeader, currentHeader, totalHeader, avgHeader };

    const legendFormat = {
      yLeftformat: this.panel.yaxes[GraphIndex.Left].format,
      yLeftDecimal: this.panel.decimals,

      yRightformat: this.panel.yaxes[GraphIndex.Right].format,
      yRightDecimal: this.panel.decimals,
    };

    const legendProps: Partial<LegendComponentProps> = {
      seriesList: this.legendData, // We are using unfiltered data to render the Legend stats
      hiddenSeries: this.ctrl.hiddenSeries,
      ...drillDownLegendState,
      ...legendOptions,
      ...valueOptions,
  ...
  addTimeAxis(options) {
    ...
    let timeFormatMode = _.isUndefined(this.panel.xaxis.timeformat) ? 'YMD' : this.panel.xaxis.timeformat;
    if (timeFormatMode === 'static') {
      timeFormatMode = this.panel.xaxis.staticTimeFormat || '%d/%b/%y';
    }
    ...
    options.xaxis = {
      ...
      label: 'Datetime',
      ticks: ticks,
      timeformat: this.time_format(ticks, min, max, timeFormatMode),
    };
  }
  ...
  addXTableAxis(options) {
    let ticks = _.map(this.data, (series, seriesIndex: number) => {
  ...
  configureAxisMode(axis, format) {
    ...
    axis.tickFormatter = (val, axis) => {
      ...
      // Moving the logic from Timeseries
      if (_.isNumber(axis.tickDecimals)) {
        return kbn.valueFormats[format](val, axis.tickDecimals, null);
      } else {
        const { datamin, datamax } = getDataMinMax(_self.data);
        const { tickDecimals, scaledDecimals } = getFlotTickDecimals(datamin, datamax, axis, _self.ctrl.height);
        const tickDecimalsPlusOne = (tickDecimals || -1) + 1;
        return kbn.valueFormats[format](val, tickDecimalsPlusOne, scaledDecimals);
      }
    };
  }

  time_format(ticks, min, max, mode) {
    // Static Mode
    if (['YMD', 'DMY', 'MDY'].indexOf(mode) === -1) {
      return mode;
    }
...
function graphDirective(timeSrv, popoverSrv, contextSrv) {
  return {
    ...
    template: '',
    link: (scope, elem) => {
      return new GraphElement(scope, elem, timeSrv, scope.ctrl.$injector);
    },
  };
}
