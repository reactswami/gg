...
export class DashboardModel {
  ...
  constructor(data, meta?) {
    ...
    this.time = data.time || { from: 'start_of_today', to: 'now', filters: '', tfid: 'Today' };
  ...
  private initMeta(meta) {
    ...
    if (this.uid && this.uid.match(/^StatseekerDefaultDrilldown.*$/)) {
      meta.canStar = false;
    }
    meta.canEdit = meta.canEdit !== false;
    meta.showSettings = meta.canEdit && !meta.isHome;
    meta.showViewMode = !meta.isHome;
    meta.canMakeEditable = meta.canSave && !this.editable;

    meta.canAddDashboard = meta.isHome && (contextSrv.isEditor || contextSrv.hasEditPermissionInFolders);
    meta.canAddFolder = meta.isHome && contextSrv.isEditor;
    meta.canImportDashboard = meta.isHome && (contextSrv.isEditor || contextSrv.hasEditPermissionInFolders);
    ...
    if (meta.embeddedMode) {
      meta.canSearch = false;
      meta.canStar = false;
      meta.canShare = false;
      meta.showSettings = false;
      meta.showViewMode = false;
    }
  ...
  repeatRow(panel: PanelModel, panelIndex: number, variable) {
    ...
    for (let optionIndex = 0; optionIndex < selectedOptions.length; optionIndex++) {
      ...
      if (panel.collapsed) {
        ...
        const insertPos: number = panelIndex + (rowPanels.length + 1) * optionIndex + 1;
        _.each(rowPanels, (rowPanel, i: number) => {
          setScopedVars(rowPanel, option);
          if (optionIndex > 0) {
  ...
  formatDate(date, format?) {
    date = moment.isMoment(date) ? date : moment.tz(date, dashboard_data.timezone);
    format = format || 'YYYY-MM-DD HH:mm:ss';
    const timezone = this.getTimezone();

    return timezone === 'browser' ? moment.tz(date, dashboard_data.timezone).format(format) : moment.utc(date).format(format);
  }
