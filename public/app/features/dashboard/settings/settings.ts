...
export class SettingsCtrl {
  ...
  /** @ngInject */
  constructor($scope, private $route, private $location, private $rootScope, private backendSrv, private dashboardSrv) {
  ...
  $onInit() {
    ...
    this.$rootScope.onAppEvent('dashboard-saved', this.onPostSave.bind(this), this.$scope);
  }
  ...
  buildSectionList() {
    ...
    // Default dashboards are not allowed to display the 'general' tab
    if (this.dashboard.uid.indexOf('StatseekerDefault') !== 0 && this.dashboard.meta.canMakeEditable) {
      this.sections.push({
        title: 'General',
        icon: 'gicon gicon-preferences',
        id: 'make_editable',
  ...
  onRouteUpdated() {
    ...
    if (this.viewId === 'settings') {
      // For default dashboards, open the permissions section first, since they dont have the 'general' section
      if (this.dashboard.uid.indexOf('StatseekerDefault') === 0) {
        this.viewId = 'permissions';
      } else if (this.dashboard.meta.canMakeEditable) {
