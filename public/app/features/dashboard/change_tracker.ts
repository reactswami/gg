...
export class ChangeTracker {
  ...
  hasChanges() {
    this.current = this.cleanDashboardFromIgnoredChanges(this.current.getSaveModelClone());
    this.original = this.cleanDashboardFromIgnoredChanges(this.original);

    const currentTimepicker = _.find(this.current.nav, { type: 'timepicker' });
    const originalTimepicker = _.find( this.original.nav, { type: 'timepicker' });

    if (currentTimepicker && originalTimepicker) {
      currentTimepicker.now = originalTimepicker.now;
    }

    const currentJson = angular.toJson(this.current, true);
    const originalJson = angular.toJson(this.original, true);
  ...
  open_logout_modal(cback = null) {
    this.callback = cback;
    this.$rootScope.appEvent('show-modal', {
      templateHtml: '<unsaved-logout-modal dismiss="dismiss()"></unsaved-logout-modal>',
      modalClass: 'modal--narrow confirm-modal',
    });
  }
  ...
  saveChanges() {
    ...
    // Don't allow callback to run when saving, because grafana will
