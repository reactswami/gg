...
export class DashboardViewState {
  ...
  getQueryStringState() {
    ...
    state.panelTitle = state.panelTitle || null;
    state.panelOffset = parseInt(state.panelOffset, 10) || null;
  ...
  syncState() {
    if (this.dashboard.meta.fullscreen) {
      ...
      if (this.state.panelId) {
        panel = this.dashboard.getPanelById(this.state.panelId);
      } else if (this.state.panelTitle) {
        const matchingPanels = this.dashboard.getPanelsByTitle(this.state.panelTitle);
        if (matchingPanels.length) {
          panel = matchingPanels[this.state.panelOffset || 0];
        }
      }
