...
export class PanelCtrl {
  ...
  $onInit() {
    ...
    this.$scope.$on('component-did-mount', () => this.panelDidMount());
  }
  ...
  getExtendedMenu() {
    ...
    if (this.canShowPanelTemplate()) {
      menu.push({
        text: 'Save as template',
        click: 'ctrl.saveTemplate()',
        role: 'Editor',
        shortcut: 't s'
      });
    }
  ...
  canShowPanelTemplate() {
    return !this.panel.fullscreen && this.panel.type!== 'dashlist';
  }
  ...
  saveTemplate() {
    saveAsTemplate(this.pluginName, this.panel, this.$injector.get('templateSrv'), this.$injector.get('backendApiSrv'));
  }
  ...
  getInfoContent(options) {
    ...
    html += new markdownit().render(interpolatedMarkdown);
    const getTitle = (link) => {
      return link.title ? '" title="' + escapeHtml(link?.title) + '" ' : '';
    };
