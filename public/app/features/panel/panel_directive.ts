...
const panelTemplate = `
  <div ng-class="{'panel-editor-container': ctrl.panel.isEditing, 'panel-height-helper': !ctrl.panel.isEditing}">
    <div panel-resizable ng-class="{'panel-editor-container__panel': ctrl.panel.isEditing, 'panel-height-helper': !ctrl.panel.isEditing}">
...
module.directive('panelResizable', ($rootScope, $document, $timeout) => {
  return {
    restrict: 'A',
    link: (scope, elem) => {
      const resizeObserver = new ResizeObserver(entries => {
        if (scope.ctrl.panel.isEditing) {
          const editHeight = Math.floor(entries[0].contentRect.height * 0.9);
          scope.ctrl.height = editHeight;
          $timeout(() => {
            scope.ctrl.render();
          });
        }
      ...
      scope.$watch('ctrl.panel.isEditing', (newState, oldState) => {
        if (!newState) {
