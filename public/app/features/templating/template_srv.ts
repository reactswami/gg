...
export class TemplateSrv {
  ...
  updateTemplateData() {
    ...
    this.index = this.variables.reduce((acc, currentValue) => {
      ...
      return acc;
    }, Object.assign(this.sessionVariables, this.urlVariables));
  }
  ...
  replace(target, scopedVars?, format?) {
    ...
    return target.replace(this.regex, (match, var1, var2, fmt2, var3, fmt3) => {
      ...
      if (scopedVars) {
        ...
        if (value) {
          const val = (fmt === 'raw' || _.isArray(value.value))  ?
            value.value : encodeURIComponent(value.value);
          return this.formatValue(val, fmt, variable);
        }
      }
  ...
  distributeVariable(value, variable) {
    value = _.map(value, (val, index: number) => {
      if (index !== 0) {
        return variable + '=' + val;
      } else {
        return val;
